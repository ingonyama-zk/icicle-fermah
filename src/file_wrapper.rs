...
impl FileWrapper {
    ...
    pub fn end_read_section(&mut self, no_check: bool) -> io::Result<()> {
        if self.reading_section.is_none() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Not reading a section",
            ));
        }

        if !no_check {
            let current_pos = self.file.stream_position().unwrap();
            let section = self.reading_section.as_ref().unwrap();
            if current_pos - section.p != section.size {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
    ...
    pub fn read_big_int(&mut self, n8: usize, pos: Option<u64>) -> io::Result<ScalarField> {
        let mut buff = vec![0u8; n8];
        if let Some(pos) = pos {
            self.file.seek(SeekFrom::Start(pos)).unwrap();
        }
        self.file.read_exact(&mut buff).unwrap();
        Ok(ScalarField::from_bytes_le(&buff))
    }

    pub fn read_wtns_header(&mut self, sections: &[Vec<Section>]) -> io::Result<Wtsn> {
        self.start_read_unique_section(sections, 1).unwrap();
        let n8 = self.read_u32_le().unwrap();
        let q = self.read_big_int(n8 as usize, None).unwrap();
        let n_witness = self.read_u32_le().unwrap();
    ...
    pub fn read_g1(&mut self) -> G1Affine {
        ...
        G1Affine::from_limbs(x, y)
    }

    pub fn read_g2(&mut self) -> G2Affine {
        let mut x = [0u8; 64];
        self.file
            .read_exact(&mut x)
            .unwrap();
